...
(() => {
  ...
  function makeBallTexture(primary, band, number) {
    ...
    ctx.font = `900 ${50 * textureScale}px "Arial Black", Arial, sans-serif`;
    [0, 256, 512].forEach(centerX => {
      ctx.beginPath();
      ctx.arc(centerX * textureScale, 128 * textureScale, 41 * textureScale, 0, Math.PI * 2);
      ctx.fillStyle = '#fffdf5';
      ctx.fill();
      ctx.lineWidth = 2.5 * textureScale;
      ctx.strokeStyle = 'rgba(23, 24, 25, .42)';
      ctx.stroke();
      ctx.fillStyle = '#111314';
      ctx.fillText(String(number), centerX * textureScale, 130 * textureScale);
    });
    const texture = new THREE.CanvasTexture(canvas);
  ...
  function updateCamera() {
    const aspect = W / H;
    const compactUiBand = clamp(H * .105, 38, 42);
    const tableEdgeAllowance = 1.18;
    const compactVisibleHeight = (worldD + tableEdgeAllowance) * H / Math.max(1, H - compactUiBand * 2);
    const compactMargin = Math.max(1.8, compactVisibleHeight - worldD);
    const compactScreenStrength = 1 - clamp((gameScale - .7) / .2, 0, 1);
    const responsiveUiMargin = lerp(1.8, compactMargin, compactScreenStrength);
    const margin = Math.max(responsiveUiMargin, ballWorldRadius * 2.2);
